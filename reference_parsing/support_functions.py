    ...
    for n,token in enumerate(page["offsets"]):
        ...
        if (page["BET_tags"][n].startswith("b") or page["BET_tags"][n].startswith("i")) and not in_ref:
            ...
            ref = {"reference_string": "", "continuation_candidate_out": continuation_candidate_out, "continuation_candidate_in": continuation_candidate_in,
                    "ref_type": generic, "contents": OrderedDict(), "order_in_page": counter, "continuation": False}
            continuation_candidate_in = False
            surface = token[0][0]
...
def process_doc(doc):
    ...
    # TODO: merging generates problems, left out for now
    """
    ...
    for r_out in cont_refs_out:
        ...
        for r_in in cont_refs_in:
            if len(r_in[1]["contents"].keys()) < 1:
                continue
            try:
                if (r_out[1]["contents"]['1']["single_page_file_number"] == r_in[1]["contents"]['1'][
                    "single_page_file_number"] - 1) and r_out[1]["ref_type"] == r_in[1]["ref_type"] and not \
                        r_out[1]["contents"][str(len(r_out[1]["contents"].keys()))]["tag"] == "pagination":
                    # MERGE
...
def json_outputter(data,threads=7):
    ...
    for doc, pc in threads.imap_unordered(process_doc, data):
        ...
        for reference in doc["references"]:
            ...
            reference["document_id"] = doc["doc_mongo_id"]
            reference["start_img_number"] = reference["contents"]["1"]["single_page_file_number"]
            reference["end_img_number"] = reference["contents"][[x for x in reference["contents"].keys()][-1]]["single_page_file_number"]
